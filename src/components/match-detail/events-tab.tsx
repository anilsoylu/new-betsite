...
function getEventConfig(event: MatchEvent): typeof EVENT_CONFIG[EventConfigKey] {
  if (event.type === "goal") {
...
function groupEventsByHalf(events: MatchEvent[]): EventGroup[] {
  ...
  for (const event of events) {
    ...
    } else if (event.minute <= 90 || (event.minute === 90 && event.extraMinute)) {
  ...
  if (secondHalf.length > 0) groups.push({ name: "2nd Half", events: secondHalf })
  if (extraTime.length > 0) groups.push({ name: "Extra Time", events: extraTime })
...
export function EventsTab({ events, homeTeam, awayTeam }: EventsTabProps) {
  ...
  const eventGroups = useMemo(() => groupEventsByHalf(sortedEvents), [sortedEvents])
    ...
    return (
      <Card>
        <CardContent className="py-12 text-center text-muted-foreground">
          <p>No events yet</p>
          <p className="text-sm mt-1">Events will appear as the match progresses</p>
        </CardContent>
      </Card>
    )
  }
        ...
        <Card className="overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-green-500/10 via-green-500/5 to-green-500/10 px-4 py-2 border-b">
            <div className="flex items-center justify-center gap-2">
              ...
              <span className="text-sm font-semibold text-green-700 dark:text-green-400">Goal Scorers</span>
            </div>
          </div>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5">
                  {goalScorers.home.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground italic">No goals</p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5">
                  {goalScorers.away.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground italic text-right">No goals</p>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className={cn("min-w-0", isHomeTeam ? "text-left" : "text-right")}>
          ...
          {event.type === "substitution" && event.relatedPlayerName && (
            <p className="text-xs text-muted-foreground">
              {isHomeTeam ? "↔ " : ""}{event.relatedPlayerName}{!isHomeTeam ? " ↔" : ""}
            </p>
          )}
