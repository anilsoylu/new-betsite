    ...
    <Card className="mt-6">
      ...
      <CardContent className="prose prose-sm dark:prose-invert max-w-none">
        ...
        <h2 className="text-xl font-bold mt-0 mb-4">
          {homeTeam.name} vs {awayTeam.name} Preview:{" "}
          {league?.name || "Football"} Clash
        </h2>
        ...
        <p>
          ...
          exciting {LeagueLink || "competitive"} encounter. Both sides will be
          looking to secure all three points as they continue their campaign
          this season.
        </p>
        ...
        {(homeLineup?.formation || awayLineup?.formation) && (
          <>
            <h3 className="text-lg font-semibold mt-6 mb-3">
              Expected Tactics & Formations
            </h3>
            <p>
              {homeLineup?.formation && awayLineup?.formation ? (
                <>
                  <strong>{homeTeam.name}</strong> are expected to line up in a{" "}
                  <strong>{homeLineup.formation}</strong> formation, while{" "}
                  <strong>{awayTeam.name}</strong> will likely deploy a{" "}
                  <strong>{awayLineup.formation}</strong> system. The tactical
                  battle between these two setups could prove decisive in
                  determining the outcome.
                </>
              ) : homeLineup?.formation ? (
                <>
                  <strong>{homeTeam.name}</strong> are expected to set up in
                  their preferred <strong>{homeLineup.formation}</strong>{" "}
                  formation for this encounter.
                </>
              ) : (
                <>
                  <strong>{awayTeam.name}</strong> will likely line up in their{" "}
                  <strong>{awayLineup?.formation}</strong> formation as they
                  look to secure a positive result on the road.
                </>
              )}
            </p>
          </>
        ...
        {referee && (
          <>
            ...
            <p>
              <strong>{referee.name}</strong> will be the referee for this
              fixture.
            </p>
          </>
        )}
        ...
        <ul className="list-disc pl-5 space-y-1">
          ...
          {homeStanding && (
            <li>
              <strong>{homeTeam.name} Position:</strong>{" "}
              {getOrdinal(homeStanding.position)} ({homeStanding.points} pts)
            </li>
          )}
          {awayStanding && (
            <li>
              <strong>{awayTeam.name} Position:</strong>{" "}
              {getOrdinal(awayStanding.position)} ({awayStanding.points} pts)
            </li>
          )}
          {h2h.length > 0 && (
            <li>
              <strong>H2H Record (Last {h2h.length}):</strong> {homeTeam.name}{" "}
              {h2hStats.homeWins}W - {h2hStats.draws}D - {h2hStats.awayWins}W{" "}
              {awayTeam.name}
            </li>
          )}
          {referee && (
...
function renderH2HAnalysis(
  ...
  if (h2hStats.homeWins > h2hStats.awayWins) {
    ...
    return (
      <p>
        These two sides have found it difficult to separate themselves in recent
        years, with <strong>{h2hStats.draws}</strong> of their last{" "}
        <strong>{h2hCount}</strong> meetings ending in stalemates. Both teams
...
function renderPrediction(
  ...
  if (homeStrength > awayStrength + 1) {
    ...
    return (
      <p>
        This promises to be a <strong>tightly contested affair</strong> with
        little separating these two sides on paper. Both teams will fancy their
        chances of victory, and the match could go either way. Expect an
...
function describeForm(stats: FormStats): string {
  ...
  if (winRate >= 0.8) {
    ...
    }, ${stats.draws} ${stats.draws === 1 ? "draw" : "draws"} and ${
      stats.losses
