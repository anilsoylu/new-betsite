...
import type { PlayerDetail, PlayerSeasonStats, PlayerTransfer, PlayerTrophy, PlayerMatch } from "@/types/football"
...
export function PlayerAboutSection({ player }: PlayerAboutSectionProps) {
  ...
  // Get career span
  const careerTeams = teams.filter(t => t.teamType === "club" || t.teamType === "domestic")
  const firstTeam = careerTeams[careerTeams.length - 1]
  const careerStart = firstTeam?.startDate ? new Date(firstTeam.startDate).getFullYear() : null

  // Get most recent transfer
  const lastTransfer = transfers[0]

  // Calculate goals per game ratio
  const goalsPerGame = careerTotals.appearances > 0
  ...
  return (
    <article className="rounded-xl border border-border bg-card p-5 space-y-5">
      ...
      <div className="prose prose-sm dark:prose-invert max-w-none space-y-3">
        <p>
          <strong>{displayName}</strong> is {age ? `${age} years old` : "a professional footballer"}
          {dateOfBirth && ` (${formatDate(dateOfBirth)})`}
          {height && `, ${height} cm tall`}
          {currentTeam && <> and plays for <strong>{currentTeam.teamName}</strong></>}.
        </p>

        {position && (
          <p>
            {displayName} plays as a <strong>{positionDisplay}</strong>
            {currentTeam?.jerseyNumber && <> and wears the number <strong>{currentTeam.jerseyNumber}</strong> jersey</>}.
            {nationality && <> The player represents <strong>{nationality.name}</strong> at international level.</>}
          </p>
        )}

        {careerTotals.appearances > 0 && (
          <p>
            Throughout their career, {firstName || displayName} has made <strong>{careerTotals.appearances} appearances</strong>,
            scoring <strong>{careerTotals.goals} goals</strong> and providing <strong>{careerTotals.assists} assists</strong>.
            {goalsPerGame && parseFloat(goalsPerGame) > 0.3 && (
              <> With a goals-per-game ratio of <strong>{goalsPerGame}</strong>, {firstName || displayName} has proven to be a consistent scorer.</>
            )}
          </p>
        )}
      </div>
      ...
      {currentSeasonStats && currentSeasonStats.appearances > 0 && (
        <section className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            ...
            {currentSeasonStats.leagueName && <> ({currentSeasonStats.leagueName})</>},
            {displayName} has made <strong>{currentSeasonStats.appearances} appearances</strong>
            {currentSeasonStats.goals > 0 && <>, scoring <strong>{currentSeasonStats.goals} goals</strong></>}
            {currentSeasonStats.assists > 0 && <> and providing <strong>{currentSeasonStats.assists} assists</strong></>}.
            {currentSeasonStats.rating && (
              <> The player maintains an average rating of <strong>{currentSeasonStats.rating.toFixed(2)}</strong>.</>
            )}
          </p>
        </section>
      )}
      ...
      {lastMatch && lastMatch.status === "finished" && (
        <section className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            {displayName}&apos;s last match was <strong>{lastMatch.homeTeamName} - {lastMatch.awayTeamName}</strong>
            {lastMatch.homeScore !== null && lastMatch.awayScore !== null && (
              <> ({lastMatch.homeScore} - {lastMatch.awayScore})</>
            )}
            {lastMatch.leagueName && <> in <strong>{lastMatch.leagueName}</strong></>}
      ...
      {lastTransfer && (
        <section className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            {displayName}&apos;s most recent transfer was from <strong>{lastTransfer.fromTeamName}</strong> to{" "}
            <strong>{lastTransfer.toTeamName}</strong>
            {lastTransfer.date && <> on {formatDate(lastTransfer.date)}</>}
            {lastTransfer.amount && lastTransfer.amount > 0 && <> for a fee of <strong>{formatTransferFee(lastTransfer.amount)}</strong></>}
            {lastTransfer.type === "loan" && <> (loan)</>}
            {lastTransfer.type === "free" && <> (free transfer)</>}.
            {transfers.length > 1 && <> The player has had <strong>{transfers.length} transfers</strong> throughout their career.</>}
          </p>
        </section>
      )}
      ...
      {totalTrophies > 0 && (
        <section className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            {displayName} has won <strong>{totalTrophies} {totalTrophies === 1 ? "trophy" : "trophies"}</strong> in their career,
            showcasing their winning mentality and contribution to team success.
          </p>
        </section>
      )}
        ...
        <div className="space-y-4">
            ...
            <div itemScope itemProp="mainEntity" itemType="https://schema.org/Question">
              ...
              <div itemScope itemProp="acceptedAnswer" itemType="https://schema.org/Answer">
                <p itemProp="text" className="text-sm text-muted-foreground mt-1">
                  <strong>{displayName}</strong> is <strong>{age} years old</strong>
          ...
          {currentTeam && (
            <div itemScope itemProp="mainEntity" itemType="https://schema.org/Question">
              ...
              <div itemScope itemProp="acceptedAnswer" itemType="https://schema.org/Answer">
                <p itemProp="text" className="text-sm text-muted-foreground mt-1">
                  <strong>{displayName}</strong> currently plays for <strong>{currentTeam.teamName}</strong>
                  {currentTeam.jerseyNumber && <> wearing the number {currentTeam.jerseyNumber} jersey</>}.
                </p>
              </div>
            </div>
          )}
          ...
          {position && (
            <div itemScope itemProp="mainEntity" itemType="https://schema.org/Question">
              ...
              <div itemScope itemProp="acceptedAnswer" itemType="https://schema.org/Answer">
                <p itemProp="text" className="text-sm text-muted-foreground mt-1">
                  <strong>{displayName}</strong> plays as a <strong>{positionDisplay}</strong>.
                </p>
              </div>
            </div>
          )}
          ...
          {height && (
            <div itemScope itemProp="mainEntity" itemType="https://schema.org/Question">
              ...
              <div itemScope itemProp="acceptedAnswer" itemType="https://schema.org/Answer">
                <p itemProp="text" className="text-sm text-muted-foreground mt-1">
                  <strong>{displayName}</strong> is <strong>{height} cm</strong> tall
          ...
          {careerTotals.goals > 0 && (
            <div itemScope itemProp="mainEntity" itemType="https://schema.org/Question">
              ...
              <div itemScope itemProp="acceptedAnswer" itemType="https://schema.org/Answer">
                <p itemProp="text" className="text-sm text-muted-foreground mt-1">
                  <strong>{displayName}</strong> has scored <strong>{careerTotals.goals} goals</strong> in{" "}
                  <strong>{careerTotals.appearances} career appearances</strong>
                  {goalsPerGame && <>, averaging {goalsPerGame} goals per game</>}.
                </p>
              </div>
            </div>
          )}
          ...
          {nationality && (
            <div itemScope itemProp="mainEntity" itemType="https://schema.org/Question">
              ...
              <div itemScope itemProp="acceptedAnswer" itemType="https://schema.org/Answer">
                <p itemProp="text" className="text-sm text-muted-foreground mt-1">
                  <strong>{displayName}</strong> is <strong>{nationality.name}</strong>.
                </p>
              </div>
            </div>
          )}
