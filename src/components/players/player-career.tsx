...
function deriveTeamsFromStats(
  ...
  for (const stat of seasonStats) {
    ...
    if (existing) {
      existing.seasons.push({ seasonId: stat.seasonId, seasonName: stat.seasonName })
    } else {
  ...
  return Array.from(teamMap.values()).map((team, index) => {
    ...
    const endYear = lastSeason?.seasonName?.match(/\d{4}/)?.[1] || lastSeason?.seasonName?.match(/\d{4}/)?.[0]
...
export function PlayerCareer({ teams, seasonStats, currentTeamId }: PlayerCareerProps) {
  ...
  const teamsToShow = derivedTeams.length > 0
  ...
  const clubCount = teamsToShow.filter(t => t.teamType === "club" || t.teamType === "domestic").length
  const nationalCount = teamsToShow.filter(t => t.teamType === "national").length
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-muted-foreground">
            {clubCount} clubs{nationalCount > 0 && `, ${nationalCount} national`}
          </span>
        </div>
      </CardHeader>
        ...
        <div className="relative">
          ...
          <div className="space-y-1">
            {sortedTeams.map((team, index) => (
              ...
              >
                {/* Timeline dot */}
                <div className={`relative z-10 w-10 h-10 rounded-full border-2 bg-background flex items-center justify-center shrink-0 ${
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    <span>
                      ...
                      {team.startDate && " - "}
                      {team.endDate ? formatTeamDate(team.endDate) : (team.startDate ? "Present" : "")}
                    </span>
                    {team.jerseyNumber && (
                      <>
