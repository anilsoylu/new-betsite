...
> = {
  ...
  2: {
    ...
    format:
      "The competition features a 36-team league table where each team plays 8 matches against different opponents, with 4 games at home and 4 away. The top 8 teams advance directly to the Round of 16, while teams ranked 9th to 24th enter a play-off round.",
  ...
  5: {
    ...
    format:
      "The competition follows a similar format to the Champions League with a league phase followed by knockout rounds.",
  ...
  8: {
    foundedYear: 1992,
    description:
      "The top tier of English football and one of the most watched leagues in the world",
    format:
      "20 teams compete in a double round-robin format, playing 38 matches each. The bottom 3 teams are relegated to the Championship.",
  ...
  564: {
    ...
    format:
      "20 teams compete in a double round-robin format, playing 38 matches each. The bottom 3 teams are relegated to Segunda Division.",
    recordHolder: "Real Madrid",
    recordTitles: 36,
    currentHolder: "Real Madrid",
    startMonth: "August",
  ...
  82: {
    ...
    format:
      "18 teams compete in a double round-robin format, playing 34 matches each. The bottom 2 teams are relegated, with the 16th-placed team entering a playoff.",
  ...
  384: {
    ...
    format:
      "20 teams compete in a double round-robin format, playing 38 matches each. The bottom 3 teams are relegated to Serie B.",
  ...
  301: {
    ...
    format:
      "18 teams compete in a double round-robin format, playing 34 matches each. The bottom 2 teams are relegated to Ligue 2.",
  ...
  203: {
    ...
    format:
      "19 teams compete in a double round-robin format, playing 38 matches each. The bottom teams are relegated to TFF First League.",
  ...
  72: {
    ...
    format:
      "18 teams compete in a double round-robin format, playing 34 matches each. The bottom team is relegated, with playoffs for the 16th and 17th places.",
    recordHolder: "Ajax",
    recordTitles: 36,
    currentHolder: "PSV",
    startMonth: "August",
  ...
  462: {
    ...
    format:
      "18 teams compete in a double round-robin format, playing 34 matches each.",
    ...
    <article className="rounded-xl border border-border bg-card p-5 space-y-5">
      ...
      <div className="prose prose-sm dark:prose-invert max-w-none space-y-3">
        <p>
          ...
          {league.type === "league" ? "league" : "competition"} in{" "}
          <strong>{countryName}</strong>.
          {metadata?.foundedYear && (
            <>
              {" "}
              Founded in <strong>{metadata.foundedYear}</strong>, it has grown
              to become one of the most prestigious competitions in world
              football.
            </>
          )}
        </p>
        ...
        {teamCount > 0 && (
          <p>
            There are overall <strong>{teamCount} teams</strong> that compete
            for the title every year, battling it out across the season for
            glory and European qualification spots.
          </p>
        )}
        ...
        {metadata?.currentHolder && metadata?.recordHolder && (
          <p>
            ...
            <strong>{metadata.currentHolder}</strong>. The team with the most
            titles in {leagueName} history is{" "}
            <strong>{metadata.recordHolder}</strong>
            {metadata.recordTitles ? (
              <>
                {" "}
                with an impressive{" "}
                <strong>{metadata.recordTitles} titles</strong>
              </>
            ) : (
              ""
            )}
      ...
      {leagueLeader && (
        <section className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            <strong>{leagueLeader.teamName}</strong> is currently leading the{" "}
            {leagueName} table with{" "}
            <strong>{leagueLeader.points} points</strong> from{" "}
            {leagueLeader.played} matches. They have recorded{" "}
            <strong>{leagueLeader.won} wins</strong>, {leagueLeader.drawn}{" "}
            draws, and {leagueLeader.lost} losses this season.
          </p>
        </section>
      )}
        ...
        <section className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            ...
            {topScorerGoals > 0 && (
              <>
                {" "}
                The Golden Boot race is heating up with the top scorer on{" "}
                <strong>{topScorerGoals} goals</strong>.
              </>
            )}
          </p>
        </section>
      ...
      {avgGoals && (
        <section className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            ...
            <strong>{avgGoals}</strong>, making it an exciting league for fans
            who love goal-filled encounters.
          </p>
        </section>
      )}
      ...
      <section className="space-y-2">
        ...
        <ul className="list-disc pl-5 space-y-1 text-sm text-muted-foreground">
          ...
          {metadata?.recordHolder && metadata?.recordTitles && (
            <li>
              <strong>Most Titles:</strong> {metadata.recordHolder} (
              {metadata.recordTitles})
            </li>
          )}
          {metadata?.startMonth && metadata?.endMonth && (
            <li>
              <strong>Season:</strong> {metadata.startMonth} -{" "}
              {metadata.endMonth}
            </li>
          )}
          {avgGoals && (
          ...
          {metadata?.recordHolder && (
            ...
            >
              ...
              >
                ...
                >
                  <strong>{metadata.recordHolder}</strong> holds the record for
                  most {leagueName} titles
                  {metadata.recordTitles
                    ? ` with ${metadata.recordTitles} championships`
                    : ""}
                  .
                </p>
              </div>
            ...
            >
              ...
              >
                <p
                  ...
                  className="text-sm text-muted-foreground mt-1"
                >
                  <strong>{metadata.currentHolder}</strong> is the reigning
                  champion of {leagueName}.
                </p>
              </div>
            </div>
          )}
            ...
            >
              ...
              >
                <p
                  ...
                  className="text-sm text-muted-foreground mt-1"
                >
                  The {leagueName} season typically runs from{" "}
                  <strong>{metadata.startMonth}</strong> to{" "}
            ...
            >
              ...
              >
                <p
                  ...
                  className="text-sm text-muted-foreground mt-1"
                >
                  <strong>{leagueLeader.teamName}</strong> is currently at the
                  top of the table with{" "}
                  <strong>{leagueLeader.points} points</strong> from{" "}
                  {leagueLeader.played} matches ({leagueLeader.won}W-
                  {leagueLeader.drawn}D-{leagueLeader.lost}L).
                </p>
              </div>
            </div>
          )}
            ...
            >
              ...
              >
                <p
                  ...
                  className="text-sm text-muted-foreground mt-1"
                >
                  <strong>{teamCount} teams</strong> compete in {leagueName}{" "}
                  each season.
                </p>
              </div>
            </div>
