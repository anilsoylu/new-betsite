...
interface RawLiveFixture {
  ...
  state_id: number;
  state?: FixtureState;
  periods?: Array<FixturePeriod>;
  scores?: Array<FixtureScore>;
  events?: Array<SportmonksEventRaw>;
  statistics?: Array<SportmonksStatisticRaw>;
}
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const response = await sportmonksRequest<Array<RawLiveFixture>>({
    ...
    if (!fixture) {
      // Fixture not in live matches - return default state
      // This is not an error - match might not have started or already finished
    ...
    const stateInfo = STATE_ID_MAP[fixture.state_id] || {
      status: "unknown",
      isLive: false,
    };
    ...
    const homeStats = (fixture.statistics || []).filter(
      (s) => s.location === "home",
    );
    const awayStats = (fixture.statistics || []).filter(
      (s) => s.location === "away",
    );
    ...
    logError("api/fixtures/[id]/live", error, {
      url: request.url,
      fixtureId: id,
    });
